...
- `skip_update_index`: This option, when populated as `true`, will skip the publishing helm index to `gh-pages` branch or `charts_repo_url`(if configured). This flag can be used when we only want to create a GitHub release for the helm chart.
